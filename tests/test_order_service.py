...
@pytest.mark.asyncio
async def test_v2_create_order_successfully(
    service,
    storage,
    order_id,
    customer_client,
    ...
    customer_client.v2_get_customer_by_email = AsyncMock(return_value=customer)
    product_client.v2_get_product_by_name = AsyncMock(return_value=product)
    storage.v2_create_order = AsyncMock(return_value=order_id)

    result = await service.v2_create_order(order_request)

    assert result == order_id

    customer_client.v2_get_customer_by_email.assert_called_once_with("ana@email.com")
    product_client.v2_get_product_by_name.assert_called_once_with("puzzle")

    storage.v2_create_order.assert_called_once()


@pytest.mark.asyncio
async def test_v2_create_order_handles_exception(
    service,
    storage,
    customer_client,
    product_client,
    ...
    storage.v2_create_order = AsyncMock(side_effect=Exception())
    customer_client.v2_get_customer_by_email = AsyncMock(return_value=customer)
    product_client.v2_get_product_by_name = AsyncMock(return_value=product)

    with pytest.raises(Exception):
        await service.v2_create_order(order_request)
